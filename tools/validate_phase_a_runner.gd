...
func _run() -> void:
	...
	assert(player.nearest_hostile(400.0) != null, "melee magnet should see live prey")
	...
	assert(player.active_economy.incoming_multiplier(player) <= 0.62, "Parovoz hull should soak hits")
	player.active_economy.set("heat", 60.0)
	assert(player.active_economy.incoming_multiplier(player) <= 0.45, "yellow heat should thicken the hull")
	assert(player.health.get_max_health() >= 120.0, "Parovoz should start with extra hull HP")
