...
func _begin_wave(index: int) -> void:
	...
	_spawn_wave(wave)
	_spawning = false
	# Empty wave or instant deaths edge case.
	if _alive_enemies <= 0:
		_on_wave_cleared()
...
## Used by smokes / debug to finish the room without killing every enemy.
...
func _show_exit() -> void:
	...
	# Enable monitoring if it's an Area2D.
...
func _on_player_died() -> void:
	# Overlay handles pause/UI; controller just stops spawning.
	_spawning = true
